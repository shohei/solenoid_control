...
**追記：**なんとソレノイドの抵抗は120Ωだった（実際は123Ωと121.5Ω）。なんとも話が簡単になる。リレーではなく、FETで動かすことにする。PWMではなく5V出力。
...
- まずソレノイドの直流抵抗を計測する
- 直流抵抗が120Ωだったらビンゴ！単純にリレーで駆動すればよい。もしリレーの１次側をマイコンの出力で直接駆動できなかったら、エミッタ接地増幅回路で補助してやる。
